...
$(document).ready(function () {
  ...
  if ($('#statistic-chart').length > 0) {
    var stat = window.dashboardChartData && window.dashboardChartData.statistic;
    var statData = stat && stat.data && stat.data.length ? stat.data : [0, 0, 0, 0, 0, 0, 0];
    var statCategories = stat && stat.categories && stat.categories.length ? stat.categories : ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    const options = {
      series: [{
        name: 'Orders',
        data: statData
...
if ($('#category-chart').length > 0) {
    var d = window.dashboardChartData && window.dashboardChartData.orderType;
    var orderTypeSeries = d && d.series && d.series.length ? d.series : [30, 20, 15, 35];
    var orderTypeLabels = d && d.labels && d.labels.length ? d.labels : ['Delivery', 'Take Away', 'Dine In', 'QR Order'];
    var options = {
...
if ($('#sales-chart').length > 0) {
    var salesPct = (window.dashboardChartData && typeof window.dashboardChartData.salesPercent === 'number') ? window.dashboardChartData.salesPercent : 40;
    salesPct = Math.min(100, Math.max(0, salesPct));
    var options = {
      series: [salesPct],
      lebels: ['Sales'],
      chart: {
...
if ($('#revenue-chart').length > 0) {
  var rev = window.dashboardChartData && window.dashboardChartData.revenue;
  var revCategories = rev && rev.categories && rev.categories.length ? rev.categories : ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  var revData = rev && rev.data && rev.data.length ? rev.data : [0, 0, 0, 0, 0, 0, 0];
  var options = {
    series: [{
    name: 'Revenue',
    data: revData
